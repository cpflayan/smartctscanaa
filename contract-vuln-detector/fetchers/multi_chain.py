...
# Etherscan V2 unified API endpoint (replaces per-chain V1 domains)
...
class MultiChainFetcher:
    ...
    def get_chain_info(self, chain: str) -> dict:
        ...
        # Resolve whether the API key is actually available
        has_api_key = False
        env_key = cfg.get("env_key", "")
        if env_key:
            ...
            key_ref = cfg["explorer_key"]
            if key_ref.startswith("${") and key_ref.endswith("}"):
                has_api_key = bool(os.environ.get(key_ref[2:-1], ""))
            else:
                has_api_key = True
